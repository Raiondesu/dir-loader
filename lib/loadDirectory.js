...
function structuredreaddir(dirname, filter, dirFilter, pathTransform, onlyEntry, isAbsolute, genFile) {
    ...
    var output = fs.readdirSync(dirname).map(function (filename) {
      ...
      if (stat.isDirectory() ) {                // if node is directory
        ...
        if (!filter || filter.test(filename)) {
          var file = genFile(path, stat, pathTransform, onlyEntry, isAbsolute, filename, location_chain);

          var output = {};
          output[filename] = file;
          return onlyEntry ? file : output;
        }
      }
...
module.exports = function (options, webpackContext) {
  ...
  function genFile(path, stat, pathTransform, isAbsolute, onlyEntry, filename, location_chain) {
  ...
  var onlyEntry = !!options.onlyEntry || false;

  if (onlyEntry && options.entryName) filter = options.entryName || filter;

  var output = structuredreaddir(abspath, filter, dirFilter, pathTransform, onlyEntry, isAbsolute, genFile);
